      ...
      <div className="relative z-10 p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1.5 min-w-0 flex-1">
            <h3 className="font-display text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight text-[var(--admin-text-hi)] truncate flex items-center gap-2" title={pedido.clienteNombre}>
              {status === "no_leido" && (
            ...
            {pedido.clienteDireccion && (
              <div className="mt-3 flex flex-col gap-1 rounded-xl bg-[var(--admin-bg)] p-3 border border-[var(--admin-border)] w-full sm:flex-row sm:items-center sm:gap-2 sm:w-fit sm:py-1.5">
                ...
                <span className="text-[11px] text-[var(--admin-text-mid)] font-semibold line-clamp-2 sm:line-clamp-1" title={pedido.clienteDireccion}>
          ...
          <div className="shrink-0 self-start sm:self-center text-left sm:text-right mt-1 sm:mt-0">
            <p className="font-display text-4xl sm:text-5xl font-black tracking-tight text-[var(--admin-text-hi)] leading-none">
              {formatCurrency(pedido.total)}
            </p>
          </div>
        </div>

        {/* Action Bar (100% Responsivo, Grilla Táctil para Móviles) */}
        <div className="mt-6 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2">
          {pedido.clienteTelefono ? (
            <a 
              href={`https://wa.me/${cleanPhone(pedido.clienteTelefono)}?text=${getWhatsAppMessage()}`}
              target="_blank"
              rel="noopener noreferrer"
              className="col-span-2 sm:flex-1 h-12 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-[#25D366] to-[#1DA851] px-3 text-xs font-black tracking-widest text-white shadow-sm transition-all hover:brightness-105 active:scale-95 whitespace-nowrap"
            >
            ...
            <div className="col-span-2 sm:flex-1 h-12 flex items-center justify-center rounded-xl bg-[var(--admin-bg)] text-[var(--admin-text-lo)] border border-[var(--admin-border)] px-3 text-xs font-black tracking-widest whitespace-nowrap">
              SIN TELÉFONO
            </div>
          )}
          ...
          <button
            ...
            className={`col-span-1 h-12 flex items-center justify-center gap-1.5 px-3.5 rounded-xl border text-xs font-black tracking-widest transition-all active:scale-95 whitespace-nowrap ${
              isViewingReceipt 
          ...
          <button
            ...
            className="col-span-1 h-12 flex items-center justify-center gap-1.5 px-3.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-bg)] text-[var(--admin-text-mid)] hover:bg-[var(--admin-input-bg)] hover:text-[var(--admin-text-hi)] text-xs font-black tracking-widest transition-all active:scale-95 whitespace-nowrap"
          >
          ...
          <button
            ...
            disabled={isUpdating}
            className="col-span-2 sm:flex-1 h-12 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-[var(--admin-accent)] to-blue-500 text-[var(--admin-sidebar-bg)] font-black tracking-widest px-3 text-xs transition-all hover:brightness-105 active:scale-95 disabled:opacity-50 whitespace-nowrap"
          >
            <span>📋 FACTURAR</span>
          </button>
          ...
          <button
            ...
            disabled={isUpdating}
            className="col-span-2 sm:col-span-auto sm:w-12 h-12 shrink-0 flex items-center justify-center rounded-xl border border-red-500/20 bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20 transition-all font-bold active:scale-95"
            title="Eliminar Pedido"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg>
            <span className="sm:hidden ml-2">ELIMINAR PEDIDO</span>
          </button>
        </div>
