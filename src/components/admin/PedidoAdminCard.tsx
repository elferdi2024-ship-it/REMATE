...
export default function PedidoAdminCard({ pedido, onViewFull }: PedidoAdminCardProps) {
  ...
  return (
    <div
      className={`group relative w-full overflow-hidden rounded-2xl sm:rounded-3xl border transition-all duration-300 text-[var(--admin-text-mid)] ${
        status === "no_leido"
          ? "border-l-[8px] border-l-red-500 border-y-[var(--admin-border)] border-r-[var(--admin-border)] bg-red-500/[0.03] shadow-[0_8px_30px_rgba(239,68,68,0.12)] animate-[pulse_3s_infinite]"
          : status === "pendiente"
          ? "border-l-[8px] border-l-amber-500 border-y-[var(--admin-border)] border-r-[var(--admin-border)] bg-amber-500/[0.015]"
          : "border-l-[8px] border-l-green-500 border-y-[var(--admin-border)] border-r-[var(--admin-border)] bg-green-500/[0.01]"
      }`}
    >
      {/* State Badge */}
      <div className="absolute right-4 top-0 z-10">
        <div className={`rounded-b-lg px-3.5 py-1 shadow-md text-[9px] font-black uppercase tracking-widest text-white transition-all ${
          status === "no_leido"
            ? "bg-[#EF233C] animate-pulse"
            : status === "pendiente"
            ? "bg-amber-500 text-white"
            : "bg-green-500 text-white"
        }`}>
          {status === "no_leido" ? "🔴 SIN LEER" : status === "pendiente" ? "📦 PREPARANDO" : "🚚 COMPLETADO"}
        </div>
      </div>
      ...
      <div className="relative z-10 p-4 sm:p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="min-w-0">
            <h3 className="font-display text-xl sm:text-2xl font-black tracking-tight text-[var(--admin-text-hi)] flex items-center gap-2" title={pedido.clienteNombre}>
              {status === "no_leido" && (
            ...
            <div className="flex items-center gap-2 flex-wrap mt-1.5">
              ...
              {pedido.sucursalId && (
                <>
                  <div className="h-1 w-1 rounded-full bg-[var(--admin-border)]" />
                  <span className="text-[9px] sm:text-[10px] font-extrabold text-[var(--admin-accent)] tracking-wider uppercase bg-[var(--admin-accent)]/10 px-2 py-0.5 rounded">
            ...
            {pedido.clienteDireccion && (
              <div className="mt-3 flex flex-col gap-1 rounded-xl bg-[var(--admin-bg)] p-3 border border-[var(--admin-border)] w-full">
                {pedido.clienteDireccion.includes("RETIRO EN LOCAL") ? (
                  <span className="text-[9px] font-black text-orange-600 tracking-wider uppercase shrink-0 dark:text-orange-400">🏬 RETIRO EN LOCAL</span>
                ) : (
                  <span className="text-[9px] font-black text-blue-600 tracking-wider uppercase shrink-0 dark:text-blue-400">🚚 ENVÍO A DOMICILIO</span>
                )}
                <span className="text-[11px] text-[var(--admin-text-mid)] font-semibold line-clamp-2" title={pedido.clienteDireccion}>
          ...
          {/* Línea Destacada del Total */}
          <div className="flex items-center justify-between bg-[var(--admin-bg)]/60 px-4 py-2.5 rounded-xl border border-[var(--admin-border)]/50">
            <span className="text-xs font-bold uppercase tracking-wider text-[var(--admin-text-lo)]">Total del Pedido:</span>
            <p className="font-display text-2xl sm:text-3xl font-black tracking-tight text-[var(--admin-text-hi)] leading-none">
              {formatCurrency(pedido.total)}
            </p>
          </div>
        </div>

        {/* Action Bar (Grilla Táctil 100% Simétrica y Estable) */}
        <div className="mt-5 grid grid-cols-4 gap-2">
          {pedido.clienteTelefono ? (
            <a 
              ...
              className="col-span-2 h-11 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-[#25D366] to-[#1DA851] px-2 text-xs font-bold text-white shadow-sm transition-all hover:brightness-105 active:scale-95 whitespace-nowrap"
            >
            ...
            <div className="col-span-2 h-11 flex items-center justify-center rounded-xl bg-[var(--admin-bg)] text-[var(--admin-text-lo)] border border-[var(--admin-border)] px-2 text-xs font-bold whitespace-nowrap">
              Sin Teléfono
            </div>
          )}
          ...
          <button
            ...
            disabled={isUpdating}
            className="col-span-2 h-11 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-[var(--admin-accent)] to-blue-500 text-[var(--admin-sidebar-bg)] font-bold px-2 text-xs transition-all hover:brightness-105 active:scale-95 disabled:opacity-50 whitespace-nowrap"
          >
            📋 Facturar
          </button>
          ...
          <button
            ...
            className={`col-span-1 h-11 flex items-center justify-center gap-1 px-1 rounded-xl border text-xs font-bold transition-all active:scale-95 whitespace-nowrap ${
              isViewingReceipt 
          ...
          <button
            ...
            className="col-span-1 h-11 flex items-center justify-center gap-1 px-1 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-bg)] text-[var(--admin-text-mid)] hover:bg-[var(--admin-input-bg)] hover:text-[var(--admin-text-hi)] text-xs font-bold transition-all active:scale-95 whitespace-nowrap"
          >
            Imprimir
          </button>
          ...
          <button
            ...
            disabled={isUpdating}
            className="col-span-2 h-11 flex items-center justify-center gap-1.5 rounded-xl border border-red-500/20 bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20 transition-all font-bold text-xs active:scale-95"
            title="Eliminar Pedido"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg>
            <span>Eliminar</span>
          </button>
        </div>
